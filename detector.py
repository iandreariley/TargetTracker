...
class SiamFC():
    ...
    """
"""self._self._design_params_params.json  environment.json  evaluation.json  hyperparams.json  hyperparams_natural.json  hyperparams_xc5.json  run.json"""
    def __init__(self, first_image, target_location):
    ...
    def _build_tracking_graph(final_score_sz, self._design_params, env):
        ...
        z_crops = extract_crops_z(frame_padded_z, npad_z, pos_x_ph, pos_y_ph, z_sz_ph,
        ...
        x_crops = extract_crops_x(frame_padded_x, npad_x, pos_x_ph, pos_y_ph, x_sz0_ph, x_sz1_ph,
                                  x_sz2_ph, self._design_params.search_sz)

        # use crops as input of (MatConvnet imported) pre-trained fully-convolutional Siamese net
        weight_file = os.path.join(env.root_pretrained, self._design_params.net)
        template_z, templates_x, p_names_list, p_val_list = _create_siamese(weight_file,
                                                                            x_crops, z_crops)
